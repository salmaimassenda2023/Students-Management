...
import { FaEdit } from "react-icons/fa"; // Using FaEdit for the edit button
    ...
    useEffect(() => {
        const fetchCurrentUserRole = async () => {
            ...
            try {
                ...
                if (!res.ok) throw new Error("Failed to fetch current user info: " + res.statusText);
                const data = await res.json();
                setCurrentUserInfo(data.user);

                // If not admin, redirect away from this page
                if (data.user.role !== 'admin') {
                    router.push('/'); // Redirect to a non-admin page, e.g., home or students list
                    // Optionally, show a "Forbidden" message before redirecting
                }
            } catch (err) {
